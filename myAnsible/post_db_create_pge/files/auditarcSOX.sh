...
# 
# Last Updated: Oct-2012 (a1ds/mxkq) 
...
--for new user auditing 
vaudsql varchar2(50); 
type dtype is varray(3) of varchar2(10); 
v3 dtype := dtype('INSERT','UPDATE','DELETE'); 
  
cursor c1(pdml varchar2) is 
select 'Audit ' || pdml || ' Table by ' || username || ' by access' newaud 
FROM dba_users u 
where  u.profile in ('PROF_DBA','PROF_BA','PROF_DEVELOPER','PROF_USER') 
and u.username not in 
  (SELECT u.username 
   FROM dba_stmt_audit_opts a, dba_users u 
   where a.user_name = u.username 
   and   u.profile in ('PROF_DBA','PROF_BA','PROF_DEVELOPER','PROF_USER') 
   and   (a.audit_option = pdml || ' TABLE')); 
...
for ixdml in v3.first..v3.last  loop 
  for ix in c1( v3(ixdml) ) loop 
    vaudsql := ix.newaud; 
    dbms_output.put_line('New User Audit: ' || vaudsql); 
    execute immediate vaudsql; 
  end loop; 
end loop; 
